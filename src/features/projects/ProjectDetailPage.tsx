...
export function ProjectDetailPage({ projectId }: { projectId: string }) {
  ...
  if (!project) {
    return (
      <ErrorState
        ...
        onRetry={() => void router.navigate({ to: '/projects' } as never)}
      />
    );
  }
  ...
  function handoff(next: PortfolioHandoff) {
    const search = {
      projectId: next.context.projectId,
      assetId: next.context.assetId,
      partnerId: next.context.partnerId,
    };
    const to =
      next.target === 'operations' ? '/events'
        : next.target === 'governance' ? '/documents'
          : next.target === 'intelligence' ? '/finance/profitability'
            : '/transactions';
  ...
  async function removeProject() {
    ...
    try {
      ...
      setDeleteOpen(false);
      await router.navigate({ to: '/projects' } as never);
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      <Button variant="ghost" onClick={() => void router.navigate({ to: '/projects' } as never)}>
