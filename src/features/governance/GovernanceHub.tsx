...
export function GovernanceHub({ onHandoff }: { onHandoff?: (handoff: GovernanceHandoff) => void }) {
  ...
  const [workspace, setWorkspace] = useWorkspaceUrlState(workspaceIds, 'documents', { parameter: 'governance_view' });
  const [selectedDocument, setSelectedDocument] = useState<Document>();
  const [formOpen, setFormOpen] = useState(false);
  const [documentPending, setDocumentPending] = useState(false);
  const [documentError, setDocumentError] = useState('');
  const [filters, setFilters] = useState({ type: 'all', projectId: '', partnerId: '', expiry: 'all' });
  ...
  return (
    <>
      <WorkspaceShell
        ...
        summaries={workspace === 'documents' ? <DocumentFilters filters={filters} locale={locale} records={records} onChange={setFilters} /> : undefined}
      >
        {workspace === 'documents' && (
          <DocumentVault
      ...
      >
        <DocumentForm
          key={`${filters.projectId}:${filters.partnerId}:${formOpen}`}
          formId="governance-document-form"
