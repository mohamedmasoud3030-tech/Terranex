...
function SettingsWorkspace({ locale }: { locale: 'ar' | 'en' }) {
  ...
  return <div className="space-y-4"><CompanySettingsForm /><div className="grid gap-4 xl:grid-cols-3"><Card><CardContent><Languages className="h-5 w-5 text-primary" /><h2 className="mt-3 font-bold">{locale === 'ar' ? 'اللغة والاتجاه' : 'Language & direction'}</h2><div className="mt-3 flex gap-2"><Button variant={locale === 'ar' ? 'primary' : 'secondary'} onClick={() => setLocale('ar')}>العربية</Button><Button variant={locale === 'en' ? 'primary' : 'secondary'} onClick={() => setLocale('en')}>English</Button></div></CardContent></Card><Card><CardContent><Palette className="h-5 w-5 text-primary" /><h2 className="mt-3 font-bold">{locale === 'ar' ? 'المظهر' : 'Appearance'}</h2><div className="mt-3 flex flex-wrap gap-2">{themes.map((item) => <Button key={item} variant={mode === item ? 'primary' : 'secondary'} onClick={() => setMode(item)}>{item}</Button>)}</div></CardContent></Card><Card><CardContent><h2 className="font-bold">{locale === 'ar' ? 'الحساب والجلسة' : 'Account & session'}</h2><p className="mt-2 break-all text-sm text-muted-foreground">{user?.email ?? '—'}</p><p className="mt-1 text-xs text-muted-foreground">{locale === 'ar' ? 'لا توجد أدوار وهمية في الواجهة؛ الصلاحيات يحددها Supabase.' : 'No invented UI roles; Supabase owns authorization.'}</p><Button variant="danger" className="mt-4" onClick={() => setConfirmSignOut(true)}><LogOut className="h-4 w-4" />{locale === 'ar' ? 'تسجيل الخروج' : 'Sign out'}</Button></CardContent></Card></div><BackupRestoreSection locale={locale} /><ConfirmDialog open={confirmSignOut} onOpenChange={setConfirmSignOut} title={locale === 'ar' ? 'تأكيد تسجيل الخروج' : 'Confirm sign out'} entityName={user?.email ?? 'Current session'} impact={locale === 'ar' ? 'ستنتهي الجلسة الحالية وتُمسح ذاكرة البيانات عند انتقال الهوية.' : 'The current session ends and identity-scoped caches will be cleared by the auth flow.'} confirmLabel={locale === 'ar' ? 'تسجيل الخروج' : 'Sign out'} cancelLabel={locale === 'ar' ? 'إلغاء' : 'Cancel'} onConfirm={signOut} /></div>;
}
