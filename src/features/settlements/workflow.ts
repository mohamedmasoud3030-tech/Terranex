...
import { settlementsStore, type SettlementInput } from './storage';
...
function reconcileObligation(obligationId: string) {
...
export function recordSettlement(obligationId: string, input: RecordSettlementInput): Settlement {
  ...
  const activeTotal = reconcileObligation(obligationId);
  ...
  const settlement = settlementsStore.create({ ...input, obligation_id: obligationId });
...
export function reverseSettlement(settlementId: string, reason: string): Settlement {
  ...
  reconcileObligation(existing.obligation_id);
  return settlement;
}
