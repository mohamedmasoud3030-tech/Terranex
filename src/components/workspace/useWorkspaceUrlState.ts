...
export function useWorkspaceUrlState(allowed: readonly string[], fallback: string, { parameter = 'workspace', replace = true }: WorkspaceUrlStateOptions = {}) {
  ...
  const select = useCallback((next: string) => { if (!allowed.includes(next)) return; const url = new URL(window.location.href); url.searchParams.set(parameter, next); window.history[replace ? 'replaceState' : 'pushState'](null, '', url); setValue(next); }, [allowed, parameter, replace]);
