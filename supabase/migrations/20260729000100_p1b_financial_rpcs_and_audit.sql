...
begin
  ...
  insert into public.settlements (
    id, obligation_id, amount_egp, payment_method, payment_reference,
    receipt_document_id, settlement_date, notes, status, origin, owner_id
  )
  select
    ...
    p_settlement->>'payment_reference',
...
begin
  ...
  insert into public.stock_adjustments (
    id, asset_id, project_id, adjustment_date, reason, quantity_delta,
    unit, notes, owner_id
  )
  select
    ...
    (p_adjustment->>'quantity_delta')::numeric,
    p_adjustment->>'unit',
    p_adjustment->>'notes',
    auth.uid()
  returning id into v_adjustment_id;

  v_result := jsonb_build_object('adjustment_id', v_adjustment_id);
