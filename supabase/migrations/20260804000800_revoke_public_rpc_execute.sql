...
-- Internal financial helpers are callable only by their owner and trusted
-- server jobs. SECURITY DEFINER callers execute as the function owner and do
-- not require an external role grant for these nested calls.
...
grant execute on function public.terranex_assert_owner(uuid) to service_role;
