...
-- Non-destructive & Idempotent: enables RLS and creates policies safely.
-- =============================================================================
...
drop policy if exists settlement_allocations_select on public.settlement_allocations;
...
drop policy if exists settlement_allocations_update on public.settlement_allocations;
create policy settlement_allocations_update on public.settlement_allocations
  for update to authenticated
  using ((select auth.uid()) = owner_id)
  with check ((select auth.uid()) = owner_id);

drop policy if exists settlement_allocations_delete on public.settlement_allocations;
...
drop policy if exists operational_events_update on public.operational_events;
create policy operational_events_update on public.operational_events
  for update to authenticated
  using ((select auth.uid()) = owner_id)
  with check ((select auth.uid()) = owner_id);

drop policy if exists operational_events_delete on public.operational_events;
