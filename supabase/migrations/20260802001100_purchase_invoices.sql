-- Migration: Purchase invoices (فواتير المشتريات)
-- Mirror of sales_invoices for supplier/vendor bills with VAT tracking.
-- Date: 2026-08-02
...
create table if not exists purchase_invoices (
  ...
  status text not null default 'draft'
    check (status in ('draft','received','paid','partial','void','overdue')),
  notes text,
...
create table if not exists purchase_invoice_lines (
  ...
  -- Inventory integration: if set, a stock movement is auto-created on receive
  inventory_item_id uuid references inventory_items(id) on delete set null,
  created_at timestamptz not null default now(),
  unique (id, owner_id)
);

create index if not exists purchase_invoices_owner_idx on purchase_invoices(owner_id, issue_date desc);
create index if not exists purchase_invoice_lines_invoice_idx on purchase_invoice_lines(invoice_id);

drop trigger if exists trg_purchase_invoices_updated on purchase_invoices;
create trigger trg_purchase_invoices_updated before update on purchase_invoices
...
create policy purchase_invoices_owner_all on purchase_invoices
  for all to authenticated using (auth.uid() = owner_id) with check (auth.uid() = owner_id);
create policy purchase_invoice_lines_owner_all on purchase_invoice_lines
  for all to authenticated using (auth.uid() = owner_id) with check (auth.uid() = owner_id);
...
-- RPC: receive bill (draft -> received/entered)
...
begin
  select owner_id, status into v_owner, v_status from purchase_invoices where id = p_invoice_id for update;
  ...
  update purchase_invoices set status = 'received', updated_at = now() where id = p_invoice_id and owner_id = v_owner;
  return p_invoice_id;
end; $$;
grant execute on function receive_purchase_invoice(text, uuid) to authenticated;

-- RPC: register payment on a purchase invoice
...
grant execute on function pay_purchase_invoice(text, uuid, numeric, uuid, date, text) to authenticated;
