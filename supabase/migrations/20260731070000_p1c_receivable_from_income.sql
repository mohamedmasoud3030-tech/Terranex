-- P1C: allow record_transaction_atomic to create a RECEIVABLE obligation
-- (someone owes the company money) from an income transaction, in addition
-- to the existing PAYABLE obligation (company owes someone) from an expense
-- transaction. Direction is now read from p_payable->>'direction' and
-- validated against the transaction's own direction, so a deferred payable
-- can never be attached to an income transaction and vice versa.
--
-- Rollback: `supabase/rollback/20260731070000_p1c_receivable_from_income_rollback.sql`
-- restores the previous hardcoded 'payable'-only behavior.
...
comment on function public.record_transaction_atomic(uuid, jsonb, jsonb) is
  'Atomically records a transaction and an optional linked obligation. Obligation direction (payable/receivable) must match transaction direction (expense/income) and defaults to payable when omitted for backward compatibility. P1C.';
