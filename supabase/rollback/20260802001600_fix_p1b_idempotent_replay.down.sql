-- 0016 rollback: re-create record_transaction_atomic in its pre-fix shape
-- (strict project_id-required-first; the forward migration was a behavioural
-- fix, but we restore the prior function so round-trip returns to pre-0016).
-- NOTE: this rollback is provided for completeness; going backwards will
-- re-introduce the sparse-replay bug, which is the original state.
