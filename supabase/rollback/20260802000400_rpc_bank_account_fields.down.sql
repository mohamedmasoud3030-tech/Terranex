-- Rollback: restore previous versions of record/update transaction atomic
-- RPCs (without bank_account_id support). The column remains on the table
-- (dropping it would risk data loss if user entered data already).

-- Re-create record_transaction_atomic without bank_account_id insert logic.
...
declare
  v_owner_id uuid; v_project_id uuid; v_transaction_id uuid; v_payable_id uuid;
...
begin
  ...
  insert into public.transactions (
    ...
    p_transaction->>'category', p_transaction->>'description',
  ...
  if p_payable is not null and p_payable <> 'null'::jsonb then
    insert into public.obligations (
      ...
      (p_payable->>'amount_egp')::numeric, 0,
  ...
  v_result := jsonb_build_object('transaction_id', v_transaction_id, 'payable_id', v_payable_id);
  perform public.terranex_audit_log(p_request_id, 'record_transaction','transaction',
    case when v_payable_id is null then array[v_transaction_id] else array[v_transaction_id,v_payable_id] end,
    jsonb_build_object('transaction',p_transaction,'payable',p_payable), v_result, v_owner_id);
  return v_result;
end; $fn$;

-- Simplified update without bank_account_id
create or replace function public.update_transaction_atomic(
  p_request_id uuid, p_transaction_id uuid, p_updates jsonb, p_payable_updates jsonb default null
) returns jsonb language plpgsql security definer set search_path = public as $fn$
declare
  v_owner_id uuid; v_cached jsonb; v_payable_id uuid; v_result jsonb;
begin
  select owner_id into v_owner_id from public.transactions where id = p_transaction_id for update;
  if not found then raise exception using errcode='P0002', message='transaction not found'; end if;
  perform public.terranex_assert_owner(v_owner_id);
  perform public.terranex_lock_financial_request(v_owner_id, p_request_id);
  v_cached := public.terranex_audit_check_idempotent(p_request_id, v_owner_id);
  if v_cached is not null then return v_cached; end if;
  if exists (select 1 from jsonb_object_keys(coalesce(p_updates,'{}'::jsonb)) k(key)
             where key <> all(array['project_id','asset_id','partner_id','operational_event_id','direction','category','description','amount','currency','fx_rate','amount_egp','transaction_date','document_id','notes']::text[])) then
  ...
  update public.transactions set
    ...
    updated_at = now()
  where id = p_transaction_id and owner_id = v_owner_id;
  if p_payable_updates is not null and p_payable_updates <> 'null'::jsonb then
    select id into v_payable_id from public.obligations
     where source_transaction_id = p_transaction_id and direction='payable' and owner_id=v_owner_id limit 1 for update;
    if v_payable_id is not null then
      update public.obligations set
        project_id = case when p_payable_updates?'project_id' then nullif(p_payable_updates->>'project_id','')::uuid else project_id end,
        ...
        updated_at = now()
      where id = v_payable_id and owner_id = v_owner_id;
    end if;
  end if;
  v_result := jsonb_build_object('transaction_id', p_transaction_id, 'payable_id', v_payable_id);
  perform public.terranex_audit_log(p_request_id,'update_transaction','transaction',
    case when v_payable_id is null then array[p_transaction_id] else array[p_transaction_id,v_payable_id] end,
    jsonb_build_object('transaction_updates',p_updates,'payable_updates',p_payable_updates), v_result, v_owner_id);
  return v_result;
end; $fn$;
