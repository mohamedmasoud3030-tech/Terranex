...
function resetWorkspace() {
  ...
  global.localStorage.setItem(DOCUMENTS_KEY, JSON.stringify([{ id: 'document-1', project_id: 'project-1', type: 'invoice', title_ar: 'فاتورة أولى', created_at: '2026-01-01T00:00:00.000Z' }]));
  global.localStorage.setItem(OBLIGATIONS_KEY, '[]');
  global.localStorage.setItem(EVENTS_KEY, '[]');
  transactionsStore.reset();
}

test('deferred expense creates payable obligation linked to transaction and invoice', () => {
  resetWorkspace();
  const created = transactionsStore.create(input({ create_payable_obligation: true, payable_due_date: '2026-02-15' }));
...
test('deferred expense requires expense direction and due date before persistence', () => {
  resetWorkspace();
  assert.throws(() => transactionsStore.create(input({ direction: 'income', create_payable_obligation: true, payable_due_date: '2026-02-15' })), /مصروف/);
  assert.throws(() => transactionsStore.create(input({ create_payable_obligation: true })), /تاريخ استحقاق/);
